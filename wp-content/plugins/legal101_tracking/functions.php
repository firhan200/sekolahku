...
add_action('wp_ajax_nopriv_legal101_login_admin', 'legal101_login_admin');
add_action('wp_ajax_legal101_login_admin', 'legal101_login_admin');

add_action('wp_ajax_nopriv_legal101_logout_admin', 'legal101_logout_admin');
add_action('wp_ajax_legal101_logout_admin', 'legal101_logout_admin');
...
function legal101_login_admin(){
    ...
    if(count($response['errors']) > 0){
        ...
        $user = $wpdb->get_row("SELECT * FROM "._tbl_administrators." WHERE email_address = '$email_address' AND password = '$encrypted_password'");
        if($user == null){
            $response['is_success'] = false;
            $response['errors'][] = 'Pengguna tidak ditemukan.';
        }
    ...
    if($response['is_success']){
        //set session
        $_SESSION[SESSION_ADMIN_ID] = $user->id;
    }
...
LOGOUT ADMIN 
*/
function legal101_logout_admin(){
    global $wpdb;
    global $table_pengguna;
